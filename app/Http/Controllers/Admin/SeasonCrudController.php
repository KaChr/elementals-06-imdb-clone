...
class SeasonCrudController extends CrudController
{
    ...
    public function store(StoreRequest $request)
    {
        ...
        $tvshowId = str_replace(" ", "+", $request->tvshow_id);

        $item = Item::findOrFail($tvshowId)->with('tvshow')->where('id', '=', $tvshowId)->get()->first();

        if ($item->tvshow) {
            $tvshowTitle = str_replace(" ", "+", $item->tvshow->title);
                ...
                if ($err) {
                    ...
                    if ($err) {
                        ...
                        if ($err) {
                            ...
                            Tvshow::find($request->tvshow_id)->update([
                            ...
                            foreach ($tvdetails->seasons as $season) {
                                ...
                                if ($season_nr == $request->season_nr) {
